...
class PDF_Creator:
    ...
    def generate_qr_code(url):
        qr_code_filename = f"./qrcodes/{PDF_Creator.get_ID_from_url(url)}.png"
    ...
    def create_pdf(data_list, filename):
        ...
        for i in range(0, len(data_list), 18):
            for j in range(18):
                if i + j < len(data_list):
                    ...
                    if (len(table_row1) == 3): 
                        ...
                        table_row1.clear()
                    PDF_Creator.generate_qr_code(spotify_id)
            ...
            table1.setStyle(PDF_Creator.table_style)
            table2 = Table(table_data2, colWidths=col_width, rowHeights=row_width)
            table2.setStyle(PDF_Creator.table_style)
...
if __name__ == "__main__":
    data_list = read_data()
    PDF_Creator.create_pdf(data_list, "music-cards.pdf")